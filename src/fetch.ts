	...
	return new Promise((resolve, reject) => {
		https.get(url, { headers }, (res) => {
			if (res.statusCode !== 200) {
				reject(
					`Failed to get response from update server (code: ${res.statusCode}, message: ${res.statusMessage})`,
				);
				return;
			}
	...
	return new Promise((resolve, reject) => {
		const request = https.get(url, (res) => {
			...
			const bar = new ProgressBar(
				`${chalk.gray("[fetch]")} [:bar] :percent of ${(totalSize / 1024 / 1024).toFixed(2)} MB (:rate/MB/s)`,
				{
					complete: "=",
					incomplete: " ",
					width: 30,
