...
module.exports = async function (argv: string[]): Promise<void> {
	...
	program
		...
		.option(
			"-g, --good <commit>",
			"commit hash of a released insiders build that does not reproduce the issue",
		)
		.option(
			"-b, --bad <commit>",
			"commit hash of a released insiders build that reproduces the issue",
		)
		.option(
			"-c, --commit <commit|latest>",
			'commit hash of a specific insiders build to test or "latest" released build (supercedes -g and -b)',
		)
		.option(
			"--releasedOnly",
