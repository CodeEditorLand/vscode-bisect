...
class Builds {
	async fetchBuildByVersion(
		runtime = Runtime.WebLocal,
		version: string,
	): Promise<IBuild> {
		const meta = await jsonGet<IBuildMetadata>(
			`https://update.code.visualstudio.com/api/versions/${version}.0-insider/${this.getBuildApiName(runtime)}/insider?released=true`,
		);
	...
	async fetchBuilds(
		...
		if (typeof goodCommit === "string") {
			...
			if (typeof candidateGoodCommitIndex !== "number") {
				if (releasedOnly) {
					throw new Error(
						`Provided good commit ${chalk.green(goodCommit)} was not found in the list of insiders builds. It is either invalid or too old.`,
					);
				} else {
		...
		if (typeof badCommit === "string") {
			...
			if (typeof candidateBadCommitIndex !== "number") {
				if (releasedOnly) {
					throw new Error(
						`Provided bad commit ${chalk.green(badCommit)} was not found in the list of insiders builds. It is either invalid or too old.`,
					);
				} else {
					return this.fetchBuilds(
						runtime,
	...
	async installBuild(
		...
		// Download
		const { url, sha256hash: expectedSHA256 } = await this.fetchBuildMeta({
			runtime,
			commit,
		});
		console.log(
		...
		if (expectedSHA256 !== computedSHA256) {
			...
			console.log(
				`${chalk.gray("[build]")} ${chalk.green("✔︎")} expected SHA256 checksum matches with download`,
			);
		}
	...
	private fetchBuildMeta({
		...
		return jsonGet<IBuildMetadata>(
			`https://update.code.visualstudio.com/api/versions/commit:${commit}/${this.getPlatformName(runtime)}/insider`,
		);
	}
