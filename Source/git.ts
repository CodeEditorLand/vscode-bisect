...
class Git {
	...
	private async init(): Promise<void> {
		...
		if (await exists(GIT_VSCODE_FOLDER)) {
			console.log(
				`${chalk.gray("[git]")} pulling latest changes into ${chalk.green(GIT_FOLDER)}...`,
			);
		...
		// Clone repo if it does not exist
			...
			console.log(
				`${chalk.gray("[git]")} cloning VS Code into ${chalk.green(GIT_VSCODE_FOLDER)} (this is only done once and can take a while)...`,
			);
