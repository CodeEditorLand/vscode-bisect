...
class Bisecter {
	async start(
		...
		goodCommitOrVersion?: string,
		badCommitOrVersion?: string,
		releasedOnly?: boolean,
	): Promise<void> {
		// Resolve commits from input
		const { goodCommit, badCommit } = await this.resolveCommits(
			runtime,
			goodCommitOrVersion,
			badCommitOrVersion,
		);
	...
	private async resolveCommit(runtime: Runtime, commitOrVersion?: string) {
		...
		throw new Error(
			`Invalid commit or version format. Please provide a valid Git commit hash or version in the format of ${chalk.green("major.minor")}.`,
		);
	}
...
`);
	...
	async tryBuild(
		build: IBuild,
		options: { forceReDownload: boolean; isBisecting: boolean },
	): Promise<BisectResponse> {
		try {
			const instance = await launcher.launch(build, options);

			const response = options.isBisecting
				? await prompts([
						{
							type: "select",
							name: "status",
			...
			if (response.status === "retry") {
				return this.tryBuild(build, {
					forceReDownload: false,
					isBisecting: options.isBisecting,
				});
			}
