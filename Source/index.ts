...
module.exports = async function (argv: string[]): Promise<void> {
	...
	program
		.addOption(
			new Option(
				"-r, --runtime <runtime>",
				"whether to bisect with a local web, online vscode.dev or local desktop (default) version",
			).choices(["desktop", "web", "vscode.dev"]),
		)
		.option(
			"-g, --good <commit|version>",
			"commit hash or version of a published insiders build that does not reproduce the issue",
		)
		.option(
			"-b, --bad <commit|version>",
			"commit hash or version of a published insiders build that reproduces the issue",
		)
		.option(
			"-c, --commit <commit|latest>",
			'commit hash of a published insiders build to test or "latest" released build (supercedes -g and -b)',
		)
		.option(
			"-v, --version <major.minor>",
			"version of a published insiders build to test, for example 1.93 (supercedes -g, -b and -c)",
		)
		.option(
			"--releasedOnly",
			"only bisect over released insiders builds to support older builds",
		)
		.option(
			"--reset",
			"deletes the cache folder (use only for troubleshooting)",
		)
		.addOption(
			new Option(
				"-p, --perf [path]",
				"runs a performance test and optionally writes the result to the provided path",
			).hideHelp(),
		)
		.addOption(
			new Option(
	...
	try {
		...
		if (opts.version) {
			if (!/^\d+\.\d+$/.test(opts.version)) {
				throw new Error(
					`Invalid version format. Please provide a version in the format of ${chalk.green("major.minor")}, for example ${chalk.green("1.93")}.`,
				);
			}

			const build = await builds.fetchBuildByVersion(
				runtime,
				opts.version,
			);
			commit = build.commit;
			...
			if (opts.commit === "latest") {
				const allBuilds = await builds.fetchBuilds(
					runtime,
					undefined,
					undefined,
					opts.releasedOnly,
		...
		if (commit) {
			await bisecter.tryBuild(
				{ commit, runtime },
				{ isBisecting: false, forceReDownload: false },
			);
		}

		// No commit provided: bisect commit ranges
		else {
			await bisecter.start(
				runtime,
				goodCommitOrVersion,
				badCommitOrVersion,
				opts.releasedOnly,
